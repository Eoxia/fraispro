...
class mod_myobject_advanced extends ModeleNumRefMyObject
{
	...
	public function info($langs)
	{
		...
		$text .= '<td class="right">'.$form->textwithpicto('<input type="text" class="flat minwidth175" name="maskvalue" value="'.getDolGlobalString('FRAISPRO_MYOBJECT_ADVANCED_MASK').'">', $tooltip, 1, 'help').'</td>';
	...
	public function getNextValue($object)
	{
		...
		// We get cursor rule
		$mask = getDolGlobalString('FRAISPRO_MYOBJECT_ADVANCED_MASK');
		...
		$numFinal = get_next_value($db, $mask, 'fraispro_myobject', 'ref', '', '', $date);
