...
class doc_generic_myobject_odt extends ModelePDFMyObject
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->fraispro->dir_output) {
			...
			$dir = $conf->fraispro->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->fraispro->dir_temp);
				if (!is_writable($conf->fraispro->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->fraispro->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'FRAISPRO_MYOBJECT_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->fraispro->dir_temp,
