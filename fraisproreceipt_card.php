...
/**
 *    \file       htdocs/modulebuilder/template/fraisproreceipt_card.php
 *    \ingroup    fraispro
 *    \brief      Page to create/edit/view fraisproreceipt
 */
...
dol_include_once('/fraispro/lib/fraispro_fraisproreceipt.lib.php');

// Load translation files required by the page
$langs->loadLangs(array("fraispro@fraispro", "other"));
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('fraispro', 'fraisproreceipt', 'read');
	$permissiontoadd = $user->hasRight('fraispro', 'fraisproreceipt', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('fraispro', 'fraisproreceipt', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('fraispro', 'fraisproreceipt', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('fraispro', 'fraisproreceipt', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->fraispro->multidir_output[isset($object->entity) ? $object->entity : 1].'/fraisproreceipt';
...
if (empty($reshook)) {
	$backurlforlist = dol_buildpath('/fraispro/fraisproreceipt_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/fraispro/fraisproreceipt_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	*/
	...
	$trackid = 'fraisproreceipt'.$object->id;
...
$title = $langs->trans("FraisproReceipt")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("FraisproReceipt"));
}
$help_url = '';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-fraispro page-card');
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("FraisproReceipt"), '', $object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	$head = fraisproreceiptPrepareHead($object);

	print dol_get_fiche_head($head, 'card', $langs->trans("FraisproReceipt"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteFraisproReceipt'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/fraispro/fraisproreceipt_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->fraispro->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('fraispro:FraisproReceipt', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$tmparray = $form->showLinkToObjectBlock($object, array(), array('fraisproreceipt'), 1);
