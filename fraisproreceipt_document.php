...
/**
 *  \file       htdocs/modulebuilder/template/fraisproreceipt_document.php
 *  \ingroup    fraispro
 *  \brief      Tab for documents linked to FraisproReceipt
 */
...
$upload_dir = null;
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->fraispro->multidir_output[$object->entity ? $object->entity : $conf->entity]."/fraisproreceipt/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('fraispro', 'fraisproreceipt', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("FraisproReceipt")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("FraisproReceipt"), -1, $object->picto);
...
$modulepart = 'fraispro';
$param = '&id='.$object->id;
//$relativepathwithnofile='fraisproreceipt/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'fraisproreceipt/'.dol_sanitizeFileName($object->ref).'/';
